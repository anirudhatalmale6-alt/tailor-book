          ...
          <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-3">
                      ...
                      {fields.map((field) => (
                        <div key={field.id}>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                            placeholder="0"
                          />
                        </div>
                      ))}
            ...
            {showAddField ? (
              <div className="border-t border-gray-100 pt-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <select
                    ...
                    className="px-2 py-2 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-900"
                  >
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="py-2 px-3 bg-gray-100 text-gray-600 rounded-lg text-xs font-medium"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full py-2 border border-dashed border-indigo-300 text-indigo-600 rounded-lg text-xs font-medium hover:bg-indigo-50 transition-colors"
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            {(['pending', 'in_progress', 'ready', 'delivered'] as const).map((s) => {
              ...
              const colors: Record<string, string> = {
                pending: form.status === s ? 'bg-yellow-50 border-yellow-300 text-yellow-700' : '',
                in_progress: form.status === s ? 'bg-blue-50 border-blue-300 text-blue-700' : '',
                ready: form.status === s ? 'bg-green-50 border-green-300 text-green-700' : '',
                delivered: form.status === s ? 'bg-gray-100 border-gray-400 text-gray-700' : '',
              };
              return (
                <button
                  key={s}
                  ...
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-colors ${
                    form.status === s ? colors[s] : 'bg-white border-gray-200 text-gray-500'
                  }`}
                >
                  {labels[s]}
                </button>
